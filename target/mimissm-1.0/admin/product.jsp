...
//取得所有name=ck的复选框
    ...
    function condtion() {
        var pname=$("#pname").val();
        var ptype=$("#ptype").val();
        var lestPrice= $("#lestPrice").val();
        var maxPrice=$("#maxPrice").val();
        $.ajax({
            url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
            type:"post",
            data:{"pname":pname,"ptype":ptype,"lestPrice":lestPrice,"maxPrice":maxPrice},
            success:function () {
...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            ...
            商品类型：<select name="ptype" id="ptype">
            <option value="-1">请选择</option>
            <c:forEach items="${ptlist}" var="pt">
                <option value="${pt.typeId}">${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="lestPrice" id="lestPrice">-<input name="maxPrice" id=maxPrice">
            <input type="button" value="查询" onclick=condtion()>
        </form>
    </div>
    <br>
    <div id="table">
...
<script type="text/javascript">
        ...
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            ...
            $.ajax({
                ...
                data: {"pid":pid,"pname":pname,"ptype":ptype,"lestPrice":lestPrice,"maxPrice":maxPrice},
                type: "post",
                dataType:"text",
    ...
    function one(pid,page){
        ...
        var lestPrice= $("#lestPrice").val();
        var maxPrice=$("#maxPrice").val();
        var str ="?pid="+pid+"&page="+page+"&pname="+pname+"&ptype="+ptype+"&lestPrice="+lestPrice+"&maxPrice="+maxPrice;
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        $.ajax({
        url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
            data:{"page":page,"pname":pname,"ptype":ptype,"lestPrice":lestPrice,"maxPrice":maxPrice},
            type:"post",
            success:function () {
                //重新加载分页显示的组件table
